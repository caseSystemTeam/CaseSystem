...
public class RoleController {
    ...
    @ResponseBody
    public ResultGson selectRoles() {
        try {

            List<Role> roleList = roleService.selectRoles(null);
            Iterator<Role> iterable = roleList.iterator();
            while (iterable.hasNext()){
                Role s = iterable.next();
                if (s.getName().equals("超级管理员")){
                    iterable.remove();
                }
            }
    ...
    @ResponseBody
    public ResultGson updateRole(@RequestBody String json, HttpSession session, HttpServletRequest request){
        User user=(User)session.getAttribute("us");
        Map<String,Object> map =  JSON.parseObject(json);
        UserRole urole = new UserRole();
        urole.setUserId((String) map.get("userId"));
        Map<String,Object> cmap=userService.getBusinessInfo(user.getBusId());
        if(cmap.get("lawerid").equals(urole.getUserId())){
            return ResultGson.error("超级管理员角色不能更改");
        }
        urole.setRoleId((String) map.get("roleId"));
        try{
            roleService.updateUserRole(urole);
