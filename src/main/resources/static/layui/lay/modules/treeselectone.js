//treeselectone组件
layui.define(['tree','layer'], function(exports) {
	"use strict";

	var _MOD = 'treeselectone',
		treeData = {}, //全局树形数据缓存
		$ = layui.jquery,
		hint = layui.hint(),
		DISABLED = 'layui-disabled',
		win = $(window),
		treeselectone = function() {
			this.v = '1.0.0';
		};
	/**
	 * 初始化下拉树选择框
	 */
	treeselectone.prototype.render = function(options) {
		var that = this,
...
//						}
			...
			init = function(treeid) {
				...
				//隐藏原控件
				othis.hide();
				othis.addClass("layui-input-treeselectone");
			...
			reset = function(filter) {
				var trees = filter ? $('*[lay-filter="' + filter + '"]') : $(".layui-input-treeselectone");
				layui.each(trees,
					function(_, one) {
						othis = $(one);
		...
		if(!othis[0])
			return hint.error('layui.treeselectone 没有找到' + options.elem + '元素');
		if(!options.data)
			return hint.error(
				'layui.treeselectone 缺少参数 data ,data 可直接指定treedata，也可以是treedata数据url，treedata参见layui tree模块');
	...
	var treeselectone = new treeselectone();
	//暴露接口
	exports(_MOD, treeselectone);
});